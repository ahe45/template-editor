...
기본 레이아웃은 ExamList 원본과 같은 데스크톱 4열 구조입니다. WorkMate처럼 앱 내부 설정 화면이나 좁은 컨테이너에 붙이는 경우에도 기본값은 4열을 유지하며, 필요하면 편집기 영역에서 가로 스크롤이 생깁니다.

작은 화면에서 toolbar, tag panel, canvas, page properties를 세로로 쌓고 싶을 때만 아래 옵션을 명시합니다.
...
`v1.0.2`에서 지원하는 adapter는 다음과 같습니다.
